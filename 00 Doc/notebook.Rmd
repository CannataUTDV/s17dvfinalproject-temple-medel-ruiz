---
title: "<center><b>2009 Renewable Energy Production Data Analysis</b></center>"
...
Despite the central role of fossil fuels in the recent 2016 election, when joining the main dataset with a dataset describing the number of wind turbines constructed in each US county between 2008 and 2014, one finds that the majority of counties with a wind turbine increase in the Obama era actually lost Democratic voters at a higher rate than the national average.
...
#**Visualizaiton #3** 
##FERNANDO MAIN:
The Barchart visualization Plots the Total Renewable energy per Housing Expesne Dollar for every State (a table calculation), which is windowed with the State's corresponding Region.
Some Regions include only one State, which generally means the State itself produces an extremely high amount of energy.
Interesting Observations:
The West region of the US has the highest Renewable Energy per Housing Expense Dollar ratio out of all the Regions with more than two States.
California is notably Excluded from the West, which would increase the average Ratio even higher. 

<br></br>
<center>![Visualizaiton #3](../03 Visualizations/RenewableEnergyperHousingExpenseDollarBar.png)</center>
<br></br>
...
#**Visualizaiton #6** 
## Election Results by Relative Strength of Renewable Energy Types:
Here we see that when counties are grouped by which which renewable resources is the most prominent compared to the its national average and color based on that State's renewable enrgy percentage one finds that several of the counties from the high percentage renewable states actually voted for Donald Trump. but one also sees that states where wind energy is highly represented did so at a much lower rate. 
<br></br>
<center>![Visualizaiton #6](../03 Visualizations/RelativeRenewables.png)</center>
<br></br>
...
The Histogram visualization uses the Total energy per Housing Expense Dollar ratio, and plots it in bins. Dot plots are used to show the average (monthly) Housing Expense per bin.
Interesting Observations:
Texas has the highest Total Energy per Housing Expense ratio.
The Dot plot shows that the average monthly housing Expense for is correlated with the Total Energy per Housing Expense ratio

<center>![Visualizaiton #6](../03 Visualizations/TotalEnergyperHousingExpenseDollarHist.png)</center>
